    ...
    for i in range(len(layers)):
        ...
        if args.is_layered:
            for name in subset:
                ...
                parts = name.split('.')
                if parts[0]=='self_attention':
                    setattr(layer.self_attention, parts[1], linear_PQ)
                    subset[name]=getattr(layer.self_attention, parts[1], None)
                else:
                    setattr(layer.mlp, parts[1], linear_PQ) 
                    subset[name]=getattr(layer.mlp, parts[1], None)
        gptq = {}
        for name in subset:
            gptq[name] = GPTQ(subset[name],args.is_layered,args.sparsity,wbits=args.wbits)
            gptq[name].quantizer = Quantizer()
            gptq[name].quantizer.configure(
                args.wbits, perchannel=True, sym=args.sym, mse=True
            )
